...
namespace RaxterBaxter.WorldStreamer
{
    public static class SceneLoadUtility
    {
        ...
        private static void LoadSceneBuildMode(AssetReference scene)
        {
        ...
        static void UnloadScenePlayModeViaSceneManager(string scene) => SceneManager.UnloadSceneAsync(scene);
        ...
        // returns true if scene was unloaded, false if canceled
        static void UnloadSceneEditorModeViaEditorSceneManager(Scene toUnload)
        {
            if (toUnload.isDirty)
            {
        ...
        [EditorModeOnly]
        static Scene GetLoadedSceneReference(AssetReference sceneReference)
        {
        ...
        static Scene GetLoadedSceneReference(string sceneName)
        {
        ...
        static string GetScenePathEditorMode(AssetReference sceneReference)
        {
        ...
        internal static AssetReference GetAssetReferenceSceneFromScene(Scene scene) {
        ...
        static AssetReference GetAssetReferenceSceneFromSceneEditMode(Scene scene)
        {
            var path = scene.path;
        ...
        static AssetReference GetAssetReferenceSceneFromScenePlayMode(Scene scene)
        {
        ...
        public static IEnumerable<Scene> AllSceneManagerScenes
        {
            get
            {
                for (int i = 0; i < SceneManager.sceneCount; i++)
        ...
        public static bool AnySceneLoading
        {
            get
            {
                foreach (var s in AllSceneManagerScenes)
    ...
    internal class EditorModeOnlyAttribute : Attribute
    {
    }

    internal class BuildModeOnlyAttribute : Attribute
    {
    }

    internal class PlayModeOnlyAttribute : Attribute
    {
    }
}